...
class Paypal extends Model
{
    ...
    {
        #Account information
        $config["business"]     = getSetting('email', 'paypal'); //$settings->email; //Account email or id
        ...
        $config["currency_code"] = getSetting('currency', 'paypal');
        //$settings->currency; //http://bit.ly/anciiH
        $config["disp_tot"] = 'Y';

        #Page Layout -->
        $config["cpp_header_image"]         = IMAGE_PATH_SETTINGS . getSetting('image', 'paypal');
        //$settings->image; //Image header url [750 pixels wide by 90 pixels high]
        ...
        $config["return"]             = URL_PAYPAL_PAYMENT_SUCCESS; //The URL to which PayPal redirects buyersâ€™ browser after they complete their payments.
        $config["cancel_return"]    = URL_PAYPAL_PAYMENT_CANCEL; //Specify a URL on your website that displays a â€œPayment Canceledâ€ page.
    ...
    public function intilizeConfig()
    {

        $this->config['business']             = $this->settings->email;
        $this->config['currency_code']         = $this->settings->currency;
        $this->config['cpp_header_image']     = $this->settings->image;
        $this->config['cpp_header_image']     = $this->settings->image;
        $this->config['return']                = URL_PAYPAL_PAYMENT_SUCCESS;
        $this->config['cancel_return']      = URL_PAYPAL_PAYMENT_CANCEL;
    }
