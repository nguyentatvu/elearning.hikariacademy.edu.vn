...
class TopicsController extends Controller
{
    ...
    public function index()
    {
      if(!checkRole(getUserGrade(8)))
      {
        ...
        $data['title']              = 'Câu hỏi Mondai';
      // return view('mastersettings.topics.list', $data);
         $view_name = 'admin.mastersettings.topics.list';
    ...
    public function getDatatable()
    {
       if(!checkRole(getUserGrade(8)))
      {
        ...
        {
          return $records->topic_name;
        })
        ->editColumn('description', function($records)
        {
          $description = preg_replace("/<img[^>]+\>/i", " ", $records->description);
          return change_furigana_admin($description);
        })
        ->removeColumn('id')
        ->removeColumn('slug')
        ->removeColumn('updated_at')
        ->editColumn('parent_id', function($records){
          return ($records->parent_id == 0) ? '<i class="fa fa-check text-success"></i>' : Topic::where('id','=',$records->parent_id)->pluck('topic_name')->first() ;
        })
        ->rawColumns(['action', 'description', 'parent_id'])
        ->make();
    }
    /**
    ...
    public function create()
    {
      ...
      // return view('mastersettings.topics.add-edit', $data);
       $view_name = 'admin.mastersettings.topics.add-edit';
        return view($view_name, $data);   
    }
    /**
    ...
    */
     public function import()
     {
        ...
        // return view('mastersettings.topics.import.import', $data);
         $view_name = 'admin.mastersettings.topics.import.import';
        return view($view_name, $data);   
     }
     ...
     catch( \Illuminate\Database\QueryException $e)
     {
       ...
       $view_name = 'admin.mastersettings.topics.import.import-result';
        return view($view_name, $data); 
     }
