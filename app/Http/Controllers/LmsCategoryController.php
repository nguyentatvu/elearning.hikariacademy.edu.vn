...
class LmsCategoryController extends Controller
{
    ...
    public function index()
    {
        if(!checkRole(getUserGrade(2)))
      {
    	...
    	// return view('lms.lmscategories.list', $data);
           ...
           $view_name = 'admin.lms.lmscategories.list';
    ...
    public function getDatatable()
    {
        if(!checkRole(getUserGrade(2)))
      {
        ...
        $records = DB::table('lmscategories')
        ->select(['category', 'image', 'description', 'id','slug'])
        ->where([
          ['delete_status',0]
        ])
        ->orderBy('id','desc')
        ...
        ->editColumn('category', function($records){
            return "<a href='".$records->slug."/series'>".$records->category."</a>";
        })
    ...
    public function create()
    {
        if(!checkRole(getUserGrade(2)))
      {
    	...
    	// return view('lms.lmscategories.add-edit', $data);
         ...
         $view_name = 'admin.lms.lmscategories.add-edit';
        return view($view_name, $data);
    }
