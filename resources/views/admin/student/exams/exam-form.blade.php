...
<div class="questions">
    <div class="col-xs-6">
     <div class="show_mondai hikari-croll-full" id="hikari-mondaiscroll">       
      ...
      <div class="mondai_subject_show" id="mondai_subject_<?php echo $key_subject_topic; ?>" style="padding-right: 8px; <?php echo $i_subject_topic!=1 ? 'display:none':'';?>" >
...
<div class="col-xs-6" style="position: relative;">
  <div class="container-answer hikari-croll-full hikari-w260" id="hikari-croll-full" style="overflow-y: scroll; overflow-x: hidden;">
    ...
    $count_questions = count($questions);
     ...
     <tr class="hik-table-tr-question">
         <td class="hik-table-tr-question-number" style="width: 52px;"><span class="question_number">3</span></td>
         <td class="hik-table-tr-question-question">
  ...
  <tr class="hik-table-tr-question">
     <td class="hik-table-tr-question-number" style="width: 52px;"><span >質問1</span></td>
     <td class="hik-table-tr-question-question">
      {{ change_furigana( trim($question->question)) }}
  </td> 
</tr>
...
<tr class="hik-table-tr-question">
 <td class="hik-table-tr-question-number" style="width: 52px;"><span>質問2</span></td>
 <td class="hik-table-tr-question-question">
  {{ change_furigana( trim($question->question)) }}
</td> 
</tr>
...
<table style="width:98%">
    @if ($number_questions <= 30)
        @if(!empty($question->explanation))
            <div class="hik-table-tr-question">{{ change_furigana($question->explanation) }}</div>
        @endif
          <tr class="hik-table-tr-question">
    ...
    @if ($number_questions == 31)
        ...
        <tr class="hik-table-tr-question">
           <td class="hik-table-tr-question-number" style="width: 70px;"><span class="question_number">3</span></td>
           <td class="hik-table-tr-question-question">
                {{ change_furigana(trim($question->explanation)) }}
            </td> 
        </tr>
        <tr class="hik-table-tr-question">
           <td class="hik-table-tr-question-number" style="width: 70px"><span >質問1</span></td>
           <td class="hik-table-tr-question-question">
            {{ change_furigana( trim($question->question)) }}
            </td> 
        </tr>
...
<tr class="hik-table-tr-question">
   <td class="hik-table-tr-question-number" style="width: 70px;"><span>質問2</span></td>
   <td class="hik-table-tr-question-question">
    {{ change_furigana( trim($question->question)) }}
</td> 
</tr>
...
function saveFormData () {
  ...
  function initTimer (isError) {
       ...
       if(!localStorage)
       {
        ...
        //  console.log("dueration: "+ data.DURATION);
         const audioStorage = window.localStorage.getItem("AUDIO_KEY");
        
        //  console.log("SECONDS ERROR TIME: "+ data.DURATION);
         ...
         else 
         {
           ...
           if(diffInMinutes >= minutes)
           {
              ...
              if(audioStorage)
              {
                ...
                momentStartTime = moment(data.START_TIME).add(secondError, 'seconds').format('YYYY-MM-DD HH:mm:ss');
              ...
              // console.log('minutes use: ' + minutes);
              // console.log('seconds use: ' + duration.seconds());
              // console.log('duration: ' + duration);
              // console.log('currentSeconds: ' + currentSeconds);

              let seconds = 60 - duration.seconds();
              minutesEnd = minutesEnd - 1;
              // console.log('seconds con lai: ' + seconds);

              // Nếu cùng thời gian thì giảm 1
              // if(minutesEnd == data.DURATION)
              // {
              //   minutesEnd = minutesEnd - 1;
              // } 
              const formatMin = minutesEnd.toString().length < 2 ? `0${minutesEnd.toString()}` : minutesEnd.toString();
              ...
              if(!isError)
              {
                ...
                if(answer && answer.length > 0)
                {
                    ...
                    $(`input[name="${element.id}[]"][value="${Number(index) + 1}"]`).prop("checked", true);
                  });
                }
              }
           }
