    @php
        ...
        $quiz1BadgeClass = $quiz1Analysis
            ...
            : 'badge-warning';
        $quiz2BadgeClass = $quiz2Analysis ? getBadgeClass($results->quiz_2_total, 60) : 'badge-warning';
        $quiz3BadgeClass = $quiz3Analysis ? getBadgeClass($results->quiz_3_total, 60) : 'badge-warning';
    @endphp
    <div>
        <h5 class="font-weight-bold">Thông tin chi tiết</h5>
        <p class="d-flex justify-contents-center align-items-center">
            <span class="">Tổng điểm:</span>
            <span class="badge badge-danger score-badge">{{ $results->total_marks }}/180</span>
        </p>
        <p>Thời gian tiến hành: {{ date_format(date_create($results->created_at), 'd-m-Y') }}</p>
        <p>
            Đánh giá: <span class="badge badge-{{ $badgeClass }} score-badge">{{ $badgeText }}</span>
        </p>
        <p>Chứng nhận</p>
    </div>
        ...
        <div id="accordion">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center" id="headingIncompetent"
                    data-toggle="collapse" data-target="#collapseIncompetent" aria-expanded="true"
                    aria-controls="collapseIncompetent">
                    <h5 class="mb-0">
                    ...
                    <div class="d-flex">
                        ...
                        <i class="fa fa-chevron-down ml-1 @if ($results->category_id < $N3) mt-4 @endif"
                            aria-hidden="true"></i>
                    </div>
                </div>
                @if ($quiz1Analysis)
            ...
            @if ($results->category_id == $N3)
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center" id="headingReading"
                        data-toggle="collapse" data-target="#collapseReading" aria-expanded="true"
                        aria-controls="collapseReading">
                        <h5 class="mb-0">
                            読解
                        </h5>
                        <div>
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center" id="headingListening"
                    data-toggle="collapse" data-target="#collapseListening" aria-expanded="true"
    ...
    <div>
        @if ($data['quiz_result_review'] != null)
            <span class="card-link">Đánh giá của giảng viên</span>
        ...
        @else
            <span class="card-link">Đánh giá của giảng viên</span>
            <textarea readonly class="form-control" name="" id="" cols="30" rows="5">Chưa có đánh giá!</textarea>
        @endif
    </div>
