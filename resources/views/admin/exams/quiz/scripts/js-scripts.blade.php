...
<script>
  app.controller('prepareQuestions', function( $scope, $http, httpPreConfig,$sce) {
    ...
    $scope.type_type = "{{$quiz_record->type}}";
    $scope.category = "{{$quiz_record->category_id}}";
    ...
    $scope.initAngData = function(data) {
      ...
      $scope.is_have_section   = dta.is_have_sections
        ...
        httpPreConfig.webServiceCallPost(route, data).then(function(result){
          ...
          $scope.subjectTopics    = result.topics;
          $scope.contentAvailable = true;
          $scope.removeDuplicates();
        });
      }
      $scope.topicChanged = function(selected_number) {
        if(selected_number=='')
          selected_number = $scope.subject_id;
        topic_id = selected_number;
        if(topic_id === undefined)
          return;
        route = '{{URL_QUIZ_TOPIC_GET_QUESTIONS}}';  
        data= {  _method: 'post', 
        '_token':httpPreConfig.getToken(),
        'topic_id': topic_id
      };
    ...
    $scope.submitForm = function(){
      ...
      if ($scope.category == 4) {
          //Nghe
          if ( $scope.type_type == 1) {
            socau = 28;
          //Tu vung
        } else if ( $scope.type_type == 2) {
          socau = 34;
          //Ngu phap
        } else if ( $scope.type_type == 3) {
          socau = 35;
        }
      }
      if ($scope.category == 5) {
      ...
      // if ($scope.savedQuestions.length < socau) {
      //   alert("Số câu hỏi chưa đủ "+ socau +" câu");
      // } else if ($scope.savedQuestions.length == socau) {
      //   $('#frm_update_question').submit();
      // } else {
      //   alert("Số câu hỏi đã vượt quá "+ socau +" câu");
      // }  
      $('#frm_update_question').submit();
    ...
    $scope.addQuestion = function(question, subject) {
        ...
        record.subject_question = subject.subject_question;
        record.question_id   = question.id;
        record.marks         = question.marks;
        record.question      = question.question;
        record.socautrung         = question.socautrung;
        ...
        @if($quiz_record->exam_type=='ST')
           ...
           //add new
           //$scope.setItem('saved_subject_question', $scope.savedQuestions);
           var number_question_add_subject = $('table#table_right .'+record.subject_id).length;
           if (number_question_add_subject >= record.subject_question) {
            alert('Số câu hỏi đã vượt quá '+ record.subject_question +' câu - ' +record.subject_title);
          }
           //alert('số lượng' + len);
           //alert(record.subject_question);
