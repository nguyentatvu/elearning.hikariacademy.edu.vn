<script src="{{ asset('/js/client/mock-exam/bootstrap-toggle.min.js') }}"></script>

<script src="{{ asset('/js/client/mock-exam/jquery.flexslider.js') }}"></script>

<script src="{{ asset('/js/client/mock-exam/angular.js') }}"></script>

<script src="{{ asset('/js/client/mock-exam/angular-messages.js') }}"></script>

<script src="{{ asset('/js/client/mock-exam/mousetrap.js') }}"></script>
...
function saveResumeExamData(current_question){
 ...
 $(inputs_data).each(function(){
  ...
  // Hoan.Phan Nếu User click khoanh đáp án, thì lưu lại vào local
  ...
  if(localStorage)
  {
    ...
    const id = question_number;
    const object = {
      "id": id,
      "data": user_answers
    }
    const index = answer.findIndex(x=>x.id == question_number);
    if(index != -1)
    {
      answer[index] = object;
    } else {
...
function showSubjectQuestion(subject_id) {
  question_number = $($(".question_div."+subject_id).first()).attr('id');
  mondai = $('.question_div .'+subject_id+'_'+question_number).html();
  //alert(question_number);
  $(".hikari-croll-full").animate({ scrollTop: 0 }, "fast");
  $('.show_mondai').html(mondai);
  $(VISIBLE_ELEMENT).hide();
  $("."+subject_id).fadeIn();

  //alert(subject_id);
  $('.mondai_subject_show').hide();
  $("#mondai_"+subject_id).fadeIn();

  doGeneralOperations();
  var count_page = $('ul.subject-page li').length;
  var page_number = $('.hikari-page.active').attr('data-page');
  if (page_number == count_page) {
    $('.hik-trang-sau').hide();
    $('.hik-trang-truoc').show();
    $('div#submit_div').html('<span class="submit-question" style="text-align: center; padding-top: 0px;"><a href="javascript:void(0);" class="btn btn-lg btn-danger button finish finish_exam" type="button" style="padding: 10px;">NỘP BÀI</a></span>');
  } 
...
function intilizetimer(hrs, mins, sec)
{
  ...
  SECONDS     = sec;
    TOTAL_TIME =MINUTES;
  $("#timerdiv").addClass('text-success-time');
  startInterval();
} 
...
function checkTimer() {
 if(AJAX_CALL_MAX_SECONDS == AJAX_CALL_TIME)
 {
   question_id = $(VISIBLE_ELEMENT).attr('id');
    ...
    function tictac(){
      SECONDS--;
        $('input[name="time"]').val((TOTAL_TIME - MINUTES)+':'+ (60- SECONDS));
      checkTimer();
      ...
      if(SECONDS<=0)
      {
        MINUTES--;
                ...
                $("#mins").text(MINUTES);
                /*if(MINUTES == MINUTES_1_2)
                {
                      alertify.alert("Bạn còn "+MINUTES_1_2+" phút để làm bài!", function(){
                    });
                  }*/
                  if(MINUTES == 9)
                  {
                  ...
                  $("#mins").text('00');
                    // $("#timerdiv #mins span").text('0');
                      //alert('Bạn đã hoàn thành bài thi ok');
...
function gach_cheo() {
  ...
  var gach_cheo = height_showmondai - height_description_mondai;
