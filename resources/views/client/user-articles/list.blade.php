            ...
            <div class="notification-block">
                <div class="notification">
                    <div class="notification-image">
                        <img height="40" width="40" src="{{ asset('images/icons/blog.png') }}" />
                    </div>
                    <h2 class="notification-title">Chưa có bài viết</h2>
                    <p class="notification-message">Hiện chưa có bài viết mới nào ở đây. Hikari sẽ sớm cập nhật những tin tức bổ ích, bạn đừng bỏ lỡ nhé!</p>
                </div>
            </div>
        @else
                    ...
                    <div class="search-section container">
                        ...
                        <div class="search-box">
                            ...
                            <form action="{{ route('user-articles.list') }}" method="get">
                                <input type="text" class="search-input" placeholder="Nhập từ khoá..." name="search"
                                    value="{{ $search }}">
                            </form>
                        </div>
                    </div>
                                ...
                                <div class="featured-item">
                                    <div class="featured-info">
                                        <a href="{{ route('user-articles.detail', $article->slug) }}">
                                            <h5 class="article-title line-clamp-2">
                                                {{ $article->title }}
                                            </h5>
                                        </a>
                                    ...
                                    <div class="featured-img">
                                        <a href="{{ route('user-articles.detail', $article->slug) }}">
                                            <img src="{{ url($article->thumbnail) }}" alt="Ảnh bài viết"
                                                class="contain-img">
                                        </a>
                                    </div>
                                </div>
            ...
            <div class="article-section container">
                <div class="article-list-container">
                    ...
                    @else
                        <h2>Danh sách bài viết theo từ khoá: <strong class="primary-color">“{{ $search }}”</strong>
                        </h2>
                    @endif
                        ...
                        @foreach ($articles as $article_index => $article)
                            @php
                                if (
                                    ...
                                    $article_index < $featured_article_number &&
                                    is_null($search)
                                ) {
                                    continue;
                                }
                        ...
                        @foreach ($categories as $category)
                            <div class="category-item">
                                <a href="{{ route('user-articles.list_by_category', $category->slug) }}">
                                    ...
                                    <i class="bi bi-arrow-right text-primary fs-3" style="line-height: 1;"></i>
                                </a>
                            </div>
                        @endforeach
                    </div>
