            ...
            <div class="course-content">
                ...
                <div class="price-roadmap-learn-container">
                    ...
                    <div class="button-container">
                        <button class="roadmap-btn" onclick="location.href='#'">
                            <div>
                        ...
                        @if (Auth::check() && $isValidPayment)
                            <button class="purchase-btn" onclick="location.href='{{ route('learning-management.lesson.show', ['combo_slug' => $seriesCombo->slug, 'slug' => request()->route('slug')]) }}'">
                                <div><i class="bi bi-book me-1"></i>Học ngay</div>
                            </button>
                        @elseif (Auth::check() && !$isValidPayment)
                            <button class="purchase-btn" onclick="location.href='{{ route('payments.lms', $seriesCombo->slug) }}'">
            ...
            <div class="d-flex content-section-wrapper">
                <div class="lesson-list-dropdown">
                    ...
                    <div class="accordion" id="accordion_container">
                        @include('client.components.series-introduction-dropdown', ['contents' => $contents])
                    </div>
                </div>
                <div class="overview-series">
                    <img src="{{ asset('/public/' . config('constant.series_combo.upload_path') . $seriesCombo->image) }}" alt="series image">
                    @if (Auth::check() && $isValidPayment)
                        <button class="btn btn-primary" onclick="location.href='{{ route('learning-management.lesson.show', ['combo_slug' => $seriesCombo->slug, 'slug' => request()->route('slug')]) }}'">
                            Học ngay
                        </button>
                    @elseif (Auth::check() && !$isValidPayment)
                        <button class="btn btn-primary" onclick="location.href='{{ route('payments.lms', $seriesCombo->slug) }}'">
                            Mua ngay
                        </button>
                    @else
        ...
        <div class="series-combo-section row">
            <div class="col-lg-8 series-item-section">
                @foreach ($seriesCombo->seriesList as $itemSeries)
                    <div class="series-card card">
                        <img src="{{ asset('/public/' . config('constant.series.upload_path') . $itemSeries->image) }}" alt="series image" class="series-image">
            ...
            <div class="col-lg-4 col-md-6 payment-card-container">
                <div class="payment-card card">
                    ...
                    <img src="{{ asset('/public/' . config('constant.series_combo.upload_path') . $seriesCombo->image) }}" alt="combo series image" class="series-combo-img">
                    ...
                    @if (Auth::check() && $isValidPayment)
                        <button class="btn btn-primary mt-2 fs-5 purchase-btn" onclick="location.href='{{ route('mypage.courses') }}'">
                            Học ngay
                        </button>
                    @elseif (Auth::check() && !$isValidPayment)
                        <button class="btn btn-primary mt-2 fs-5 purchase-btn" onclick="location.href='{{ route('payments.lms', $seriesCombo->slug) }}'">
                            Mua ngay
                        </button>
                    @else
                    ...
                    <div class="swiper-slide">
                        <div class="course-card">
                            ...
                            <div class="course-card-body">
                                ...
                                <p class="course-card-price">{{ formatCurrencyVND($recommended_series->cost) }}</p>
                                <div class="course-card-description line-clamp-3">{!! $recommended_series->short_description !!}</div>
                                <div class="course-card-teacher text-muted w-100 mb-1">{!! $recommended_series->description['teacher_description'] ?? '' !!}</div>
                                <div class="d-flex align-items-center text-primary-color mt-3">
                                    ...
                                    <span class="ms-2">{{ empty($recommended_series->chapter_count) ? 1 : $recommended_series->chapter_count }} chương</span>
